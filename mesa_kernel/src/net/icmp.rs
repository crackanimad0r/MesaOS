...
pub fn handle_icmp(data: &[u8], src_ip: [u8; 4], ttl: u8) {
    ...
    match icmp_type {
        ...
        0 => {
            ...
            crate::serial_println!(
                "[ICMP] Echo reply from {}.{}.{}.{} id={} seq={} ttl={}",
                src_ip[0],
                src_ip[1],
                src_ip[2],
                src_ip[3],
...
pub fn send_ping(dest_ip: [u8; 4], id: u16, seq: u16) -> Result<(), &'static str> {
    ...
    crate::serial_println!(
        "[ICMP] Outbound Echo Request to {}.{}.{}.{} (seq={}) checksum: {:#x}",
        dest_ip[0],
        dest_ip[1],
        dest_ip[2],
        dest_ip[3],
    ...
    loop {
        ...
        // Check if we got a reply
        {
            ...
            if let Some(pos) = replies
                .iter()
                .position(|(_, reply_id, reply_seq, _)| *reply_id == id && *reply_seq == seq)
            {
