...
// Store (TcpHeader, payload, src_ip) for TCP packet inspection
pub static mut LAST_TCP_PACKETS: Vec<(TcpHeader, Vec<u8>, [u8; 4])> = Vec::new();

pub fn handle_tcp(data: &[u8], src_ip: [u8; 4]) {
    if let Some((header, payload)) = TcpHeader::from_bytes(data) {
        // Push to the shared packet buffer for connect/accept inspection
        unsafe {
            LAST_TCP_PACKETS.push((header, payload.to_vec(), src_ip));
        }
        // Also try to deliver data to established TCP sockets
        // Only deliver if it has data (PSH flag or payload > 0) and ACK set
        if !payload.is_empty() && (header.flags & TCP_FLAG_ACK) != 0 {
            let _ = crate::linux_compat::socket::tcp_deliver(
                header.dest_port,
                header.src_port,
                src_ip,
                payload,
        ...
        // Handle pure ACK for established detection (empty data, ACK set)
        if payload.is_empty()
...
/// Check if we received a SYN-ACK for an outgoing connection
pub unsafe fn check_synack(local_port: u16, remote_port: u16, remote_ip: [u8; 4]) -> Option<u32> {
    let r_ip = u32::from_be_bytes(remote_ip);
    let mut i = 0;
    while i < LAST_TCP_PACKETS.len() {
        let (hdr, _, src_ip) = &LAST_TCP_PACKETS[i];
        let s_ip = u32::from_be_bytes(*src_ip);
        if hdr.dest_port == local_port
            && hdr.src_port == remote_port
            && s_ip == r_ip
            && hdr.flags & (TCP_FLAG_SYN | TCP_FLAG_ACK) == (TCP_FLAG_SYN | TCP_FLAG_ACK)
        {
            let ack = hdr.ack_num;
            LAST_TCP_PACKETS.remove(i);
            return Some(ack);
        }
        i += 1;
    }
...
/// Check for incoming SYN on a listening port, returns (header, payload, src_ip)
pub unsafe fn check_syn(local_port: u16) -> Option<(TcpHeader, Vec<u8>, [u8; 4])> {
    let mut i = 0;
    while i < LAST_TCP_PACKETS.len() {
        let (hdr, payload, src_ip) = &LAST_TCP_PACKETS[i];
        if hdr.dest_port == local_port
            && (hdr.flags & TCP_FLAG_SYN) != 0
            && (hdr.flags & TCP_FLAG_ACK) == 0
        {
            let result = (*hdr, payload.clone(), *src_ip);
            LAST_TCP_PACKETS.remove(i);
            return Some(result);
        }
        i += 1;
...
/// Check for ACK completing the three-way handshake for an accepted connection
pub unsafe fn check_ack(local_port: u16, remote_port: u16, remote_ip: [u8; 4]) -> bool {
    let r_ip = u32::from_be_bytes(remote_ip);
    let mut i = 0;
    while i < LAST_TCP_PACKETS.len() {
        let (hdr, _, src_ip) = &LAST_TCP_PACKETS[i];
