...
pub fn handle_packet(data: &[u8]) {
    ...
    while i < data.len() {
        ...
        match opt {
            ...
            OPT_ROUTER => {
                if len >= 4 {
                    router.copy_from_slice(&val[0..4]);
                }
            }
            OPT_SUBNET_MASK => {
                if len >= 4 {
                    subnet.copy_from_slice(val);
                }
            }
            OPT_DNS => {
                if len >= 4 {
                    dns.copy_from_slice(&val[0..4]);
                }
            }
            _ => {}
        }
    ...
    match msg_type {
        MSG_OFFER => {
            crate::serial_println!(
                "[DHCP] Received OFFER: IP {} from Server {}",
                format_ip(your_ip),
                format_ip(server_id)
            );
...
fn send_request(requested_ip: [u8; 4], server_id: [u8; 4]) {
    ...
    let _ = crate::net::udp::send_packet(
        [255, 255, 255, 255],
        DHCP_CLIENT_PORT,
        DHCP_SERVER_PORT,
        &packet,
