...
pub fn load_profile(name: &str) -> Result<(), String> {
    ...
    let config =
        crate::fs::read_to_string(&path).map_err(|e| format!("Read error: {}", e.as_str()))?;
...
fn apply_config(config: &str) -> Result<(), String> {
    ...
    if config == "dhcp" {
        ...
        crate::net::dhcp::send_discover().map_err(|e| format!("DHCP error: {}", e))
        ...
        if parts.len() == 4 {
            ...
            if let (Some(ip), Some(mask), Some(gw)) =
                (parse_ip(parts[1]), parse_ip(parts[2]), parse_ip(parts[3]))
            {
