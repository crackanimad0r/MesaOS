...
impl FileSystem for RamFs {
    ...
    fn symlink(&self, target: &str, link_path: &str) -> FsResult<()> {
        let mut root = self.root.write();
        let components = Self::parse_path(link_path);
        if components.is_empty() {
            return Err(FsError::InvalidPath);
        }
        let uid = crate::users::current_uid();
        ...
        match parent {
            FsNode::Directory { children, meta } => {
                ...
                meta.modified = crate::curr_arch::get_ticks();
                let mut node = FsNode::new_file(target.as_bytes().to_vec(), uid, gid);
                if let FsNode::File { meta, .. } = &mut node {
                    meta.node_type = super::NodeType::Symlink;
                }
    ...
    fn link(&self, target: &str, link_path: &str) -> FsResult<()> {
        let data = self.read(target)?;
        let meta = self.stat(target)?;
        if meta.node_type == super::NodeType::Directory {
            return Err(FsError::IsADirectory);
        }
        self.write(link_path, &data)
    }
}
