...
use super::ui::{palette, UiRenderer};
use alloc::format;
use core::fmt;
use spin::Mutex;
...
impl Console {
    ...
    pub fn update_status_bar(
        ...
        let battery_text = if bat_present {
            let charging_str = if bat_charging { "+" } else { "" };
            format!("{}{}%", bat_percent, charging_str)
        } else {
            format!("--%")
        };
        ...
        let status_text = format!(
            ...
            battery_text, used_mb, total_mb, disk_used, disk_total, cpu_count, hour, minute, second
        );
...
pub fn update_status_bar(
    ...
    crate::curr_arch::disable_interrupts();
    CONSOLE.lock().update_status_bar(
        used_mb,
        total_mb,
        cpu_count,
        hour,
