...
impl Rtl8139 {
    ...
    pub fn poll_rx(&mut self) -> Option<Vec<u8>> {
        unsafe {
            ...
            crate::serial_println!(
                "[NET] Rx packet available (offset={}, cbr={})",
                self.rx_offset,
                cbr
            );
            ...
            let read_u32_wrap = |offset: u16| -> u32 {
                ...
                if off <= 8192 - 4 {
                    ...
                    for i in 0..4 {
                        ...
                        bytes[i] = unsafe {
                            core::ptr::read_volatile((rx_virt_base + b_off) as *const u8)
                        };
                    }
            ...
            for i in 0..packet_len {
                ...
                packet
                    .push(unsafe { core::ptr::read_volatile((rx_virt_base + b_off) as *const u8) });
            }
