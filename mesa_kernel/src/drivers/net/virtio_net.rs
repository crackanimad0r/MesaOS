...
unsafe fn setup_queue(io_base: u32, qidx: u16, is_rx: bool) -> Result<VirtQueue, &'static str> {
    ...
    if qsize == 0 || qsize > 512 {
        return Err("Tamaño de cola invalido");
    }
...
pub fn poll_rx() -> Option<Vec<u8>> {
    unsafe {
        ...
        core::ptr::write_volatile(
            q.avail_ptr_virt.add(2 + (cur_avail_idx % q.size) as usize),
            elem_id,
        );
...
unsafe fn outb(p: u32, v: u8) {
    Port::new(p as u16).write(v);
}
unsafe fn outw(p: u32, v: u16) {
    Port::new(p as u16).write(v);
}
unsafe fn outl(p: u32, v: u32) {
    Port::new(p as u16).write(v);
}
unsafe fn inb(p: u32) -> u8 {
    Port::new(p as u16).read()
}
unsafe fn inw(p: u32) -> u16 {
    Port::new(p as u16).read()
}
unsafe fn inl(p: u32) -> u32 {
    Port::new(p as u16).read()
}
