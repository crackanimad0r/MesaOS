    ...
    for dev in devices() {
        if dev.class_code == 0x06 && dev.subclass == 0x04 {
            ...
            if secondary_bus == bus {
                crate::mesa_println!(
                    "[PCI] Bridge found: {:02x}:{:02x}.{} -> controls bus {:02x}",
                    dev.bus,
                    dev.device,
                    dev.function,
                    bus
                ...
                crate::mesa_println!(
                    "[PCI]   Bridge window opened: {:#010x} - {:#010x}",
                    base,
                    (base + effective_size - 1)
                );
...
pub fn init() {
    #[cfg(target_arch = "x86_64")]
    {
        ...
        crate::serial_println!(
            "[PCI] Scan complete: {} devices found",
...
pub fn find_hda_controller() -> Option<PciDevice> {
    let mut hda_devs = alloc::vec::Vec::new();

    // Recopilar todos los controladores HDA (Class 0x04, Subclass 0x03)
    for d in PCI_DEVICES.lock().iter() {
        if d.class_code == 0x04 && d.subclass == 0x03 {
            hda_devs.push(d.clone());
        }
    }

    if hda_devs.is_empty() {
        return None;
    }

    // Si hay más de uno, intentar evitar los de GPU (HDMI Audio) que normalmente
    // no están conectados a los altavoces del portátil/PC.
    // NVIDIA = 0x10DE, ATI/AMD Graphics = 0x1002
    if hda_devs.len() > 1 {
        for d in &hda_devs {
            if d.vendor_id != 0x10DE && d.vendor_id != 0x1002 {
                crate::mesa_println!(
                    "[PCI] HDA Preferido seleccionado: {:02x}:{:02x}.{} (Vendor: {:#06x})",
                    d.bus,
                    d.device,
                    d.function,
                    d.vendor_id
    ...
    // Fallback: retornar el primero encontrado
    Some(hda_devs[0].clone())
}
...
pub fn pci_disable_aspm(bus: u8, device: u8, function: u8) {
    ...
    if let Some(pcie_cap_offset) = pci_find_capability(bus, device, function, 0x10) {
        ...
        if (link_ctrl & 0xFFFF) != (new_ctrl & 0xFFFF) {
            crate::mesa_println!(
                "[PCI] Hard-disabling ASPM/CLKREQ for {:02x}:{:02x}.{}",
                bus,
                device,
                function
            );
...
pub fn pci_wait_for_link(bus: u8, device: u8, function: u8, retries: usize) -> bool {
    for i in 0..retries {
        ...
        if id != 0xFFFFFFFF && id != 0 {
            if i > 0 {
                crate::mesa_println!("[PCI] Link up after {} retries", i);
            }
            return true;
        }
